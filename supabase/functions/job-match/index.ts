...
serve(async (req: Request) => {
  ...
  console.log(`[${requestId}] Starting job-match request`);
...
For each job, provide a match percentage (0-100) based on skills alignment, experience level match, location compatibility, and job description relevance.`;
    ...
    return createStandardResponse({ matches: fallbackMatches }, requestId);
  } catch (error) {
    console.error(`[${requestId}] job-match error:`, error);
